...
function opt = getOptionVisLocaliser2()
    %
 ...
 if nargin < 1
  ...
  opt.subjects ={'004'};%{'001','002','003','004','005','006','007','008','009','010','011','013',...
%       '014','015','016','017','pil004','pil005'};% 'pil004','pil005', '012',
...
%     univariate
    opt.model.file =  ...
        fullfile(fileparts(mfilename('fullpath')), '..', ...
                 'model', 'model-visualLocalizer2_smdl.json');
%      opt.model.file =  ...
%     fullfile(fileparts(mfilename('fullpath')), '..', ...
%              'model', 'modelNoResponse-visualLocalizer2_smdl.json');
  ...
  opt.result.Steps(1).Contrasts(2).Name = 'static_gt_motion';
  opt.result.Steps(1).Contrasts(2).MC =  'none';
  opt.result.Steps(1).Contrasts(2).p = 0.001;
  opt.result.Steps(1).Contrasts(2).k = 0;
  
  opt.result.Steps(1).Contrasts(3).Name = 'motion_gt_static';
  opt.result.Steps(1).Contrasts(3).MC =  'FWE';
  opt.result.Steps(1).Contrasts(3).p = 0.05;
  opt.result.Steps(1).Contrasts(3).k = 0;
  
  opt.result.Steps(1).Contrasts(4).Name = 'motion_gt_static';
  opt.result.Steps(1).Contrasts(4).MC =  'none';
  opt.result.Steps(1).Contrasts(4).p = 0.001;
  opt.result.Steps(1).Contrasts(4).k = 0;
  
  opt.result.Steps(1).Contrasts(5).Name = 'motion_gt_static';
  opt.result.Steps(1).Contrasts(5).MC =  'none';
  opt.result.Steps(1).Contrasts(5).p = 0.01;
  opt.result.Steps(1).Contrasts(5).k = 0;
  
  opt.result.Steps(1).Contrasts(6).Name = 'motion_gt_static';
  opt.result.Steps(1).Contrasts(6).MC =  'none';
